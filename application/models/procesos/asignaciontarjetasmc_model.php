<?php
class AsignacionTarjetasMC_Model extends CI_model
{
	public function guardar($intTarjetaID,$strResidenteID,$strCompradorID,$strFecha,$strFechaVence,$intTipoLectura,$strFolioContrato,$dblPrecio,$bolUtilizoSeguro,$intEstatusID){
		$datos = array(
	...
	public function modificar($intAsignacionID,$strFecha,$strFechaVence,$intTipoLectura,
	...
	public function filtro($strResidenciaID, $strBusqueda, $intNumRows, $intPos){
		...
		$this->db->select("RT.asignacion_id,T.lectura, (CASE T.tipo_id
							...
							RT.tarjeta_id,DATE_FORMAT(RT.fecha,'%d-%m-%Y') AS fecha ", FALSE);
	...
	public function buscar($intAsignacionID){
		$this->db->select("RT.asignacion_id,RT.tarjeta_id, DATE_FORMAT(RT.fecha,'%d-%m-%Y') AS fecha, DATE_FORMAT(RT.fecha_vencimiento,'%d-%m-%Y') AS fecha_vencimiento,
