...
class RegistrosAccesos_Model extends CI_model
{
	public function guardar($intEmpleadoID,$intPrivadaID,$intResidenciaID,$intTipoGestionID,$strSolicitanteID,$strObservaciones,$strDuracion,$strImagen,$intEstatus){
			                ...
			                $arrDatos2 = array('fecha_modificacion' => date('Y-m-d H:i:s'));
                                        $this->db->where('usuario_id', $this->session->userdata('usuario_id'));
			                $this->db->update('usuarios',$arrDatos2);
		$datos = array(
	...
	/*public function filtro_consultaAnt($strFechaHoraInicio,$strFechaHoraFin,$intPrivadaID,$intResidenciaID,$strSolicitante,
		...
		if($strSolicitante != "")
		{
			...
			//$this->db->where("RA.solicitante_id", $strSolicitante);
            $this->db->where("(CONCAT_WS(' ','R -',RR.nombre,RR.ape_paterno,RR.ape_materno) LIKE '%$strSolicitante%' 
		...
		$this->db->join('privadas P','P.privada_id = RA.privada_id','inner');
		$this->db->join('residencias R','R.residencia_id = RA.residencia_id','inner');
        $this->db->join('residencias_residentes RR','RR.residente_id = RA.solicitante_id','left');
        $this->db->join('residencias_visitantes RV','RV.visitante_id = RA.solicitante_id','left');
        $this->db->join('registros_generales RG','RG.registro_general_id = RA.solicitante_id','left');
	...
	public function filtro_consulta($strFechaHoraInicio,$strFechaHoraFin,$intPrivadaID,$intResidenciaID,$strSolicitante,
		                           $intOperadorID,$intTipoGestionID,$intEstatusID, $intNumRows, $intPos) {
       ...
       //Si existe id de la residencia (buscar datos por residencia)
       if($intResidenciaID != 0)
       {
			$strRestricciones = "AND RA.residencia_id = $intResidenciaID ";
        ...
        //Si existe nombre del solicitante
       if($strSolicitante != "")
        {
       ...
       //Si existe id del operador
        if($intOperadorID != 0)
		{
		    $strRestricciones .= "AND RA.empleado_id = $intOperadorID ";
			
		}
        //Si existe id de la privada
		if($intPrivadaID != 0)
		{
			$strRestricciones .= "AND RA.privada_id = $intPrivadaID ";
			
		}
		//Si existe id del tipo de gestión
		if($intTipoGestionID != 0)
		{
			$strRestricciones .= "AND RA.tipo_gestion_id = $intTipoGestionID ";
			
		}
       //Si existe id del estatus
		if($intEstatusID != 0)
		{
			$strRestricciones .= "AND RA.estatus_id = $intEstatusID ";
        ...
        //Realizar consulta para obtener registros que coinciden con los criterios de búsqueda
	    $strSQLCon= $this->db->query("SELECT    DATE_FORMAT( RA.fecha_modificacion,'%d-%m-%Y %r') AS fecha, P.descripcion AS privada,
			    								R.nro_casa, R.calle, ( SELECT CONCAT_WS(' ',nombre,ape_paterno,ape_materno) AS nom 
											               			   FROM registros_generales 
													                   WHERE registro_general_id = RA.solicitante_id 
													                   UNION 
													                   ...
													                   UNION 
											                             ...
											                             FROM residencias_residentes 
													                     WHERE residente_id = RA.solicitante_id 
						 							                 ) AS solicitante, RA.OCR, (CASE RA.tipo_gestion_id 
																            			When 1 Then 'No concluida' 
																	        			When 2 Then 'Moroso' 
																	        			When 3 Then 'Proveedor' 
																						When 4 Then 'Residente' 
																						...
																						When 9 Then 'Visita de Morosos' 
					               										  				END) AS tipo_gestion, 
		                                        CONCAT_WS(' ',E.nombre,E.ape_paterno,E.ape_materno) AS operador, RA.imagen,  
		                                        (CASE RA.imagen 
		                          				 WHEN 'No fue subida!' THEN TRUE
		                          				 WHEN NOT 'No fue subida!' THEN NULL
		                          				 END) AS subida, (CASE RA.estatus_id
																  ...
																  WHEN 2 THEN 'RECHAZADO'
																  WHEN 3 THEN 'INFORMADO'
																  END) AS estado,RA.registro_acceso_id,S.registro_acceso_id AS supervicion
                                      
										
										FROM ((((((registros_accesos RA INNER JOIN privadas P ON P.privada_id = RA.privada_id ) 
				                     	INNER JOIN residencias R ON R.residencia_id = RA.residencia_id ) 
				      					INNER JOIN empleados E ON RA.empleado_id = E.empleado_id)
	        							LEFT JOIN supervicion_llamadas S ON  RA.registro_acceso_id = S.registro_acceso_id)
										...
										WHERE RA.fecha_modificacion >= STR_TO_DATE('$strFechaHoraInicio','%d-%m-%Y %H:%i:00')
										 	  AND RA.fecha_modificacion <= STR_TO_DATE('$strFechaHoraFin','%d-%m-%Y %H:%i:00')
									    $strRestricciones
									    ORDER BY RA.fecha_modificacion ASC
										LIMIT $intPos,$intNumRows");	
		$arrResultado["rows"] =$strSQLCon->result();

       
      
        //Realizar consulta para regresar el número de registros que coinciden con los criterios de búsqueda
        $strSQLCont="  SELECT COUNT(RA.registro_acceso_id) AS total
										 FROM ((((((registros_accesos RA INNER JOIN privadas P ON P.privada_id = RA.privada_id ) 
					                     INNER JOIN residencias R ON R.residencia_id = RA.residencia_id ) 
				      					 INNER JOIN empleados E ON RA.empleado_id = E.empleado_id)
										 LEFT JOIN supervicion_llamadas S ON  RA.registro_acceso_id = S.registro_acceso_id)
										 ...
										 $strRestricciones
										LIMIT 0,1";
        $objResult = mysql_query($strSQLCont);
        $intTotal=0;
        while($row= mysql_fetch_object($objResult)){
        	$intTotal= $row->total;
        }
         $arrResultado["total_rows"]=$intTotal;
