...
class OrdenesServicio_Model extends CI_model
{
	...
	public function filtro($strFechaIni,$strFechaFin,$intEstatusID,$intNumRows,$intPos){
		...
		$this->db->select("O.orden_servicio_id, DATE_FORMAT(O.fecha,'%d-%m-%Y') AS fecha, O.folio, 
	...
	public function buscar($busqueda, $tipo = 1){
		$this->db->select("O.orden_servicio_id,O.folio,DATE_FORMAT(O.fecha,'%d-%m-%Y %H:%i') AS fecha, O.estatus_id,
			               ...
			               DATE_FORMAT(O.fecha_asistio,'%d-%m-%Y %H:%i') AS fecha_asistio, 
			               DATE_FORMAT(O.cierre_fecha,'%d-%m-%Y %H:%i') AS cierre_fecha,
