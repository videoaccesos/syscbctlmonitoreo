...
class RegistrosGenerales_Model extends CI_model
{
	public function guardar($strRegistroGeneralID,$strApePaterno,$strApeMaterno,$strNombre,$strTelefono,$strCelular,$strFechanac,$strEmail,$strObservaciones,$intEstatusID){
		...
		$datos = array(
			...
			'telefono' => strtoupper($strTelefono),
			'celular' => strtoupper($strCelular),
			'fechanac' =>$strFechanac,
			'email' => strtoupper($strEmail),
			'observaciones' => strtoupper($strObservaciones),
			'estatus_id' => $intEstatusID,
			'usuario_id' => $this->session->userdata('usuario_id'));
		/*$this->db->query(" CALL gear_data_registros_generales ( '$strRegistroGeneralID', '$strApePaterno', '$strApeMaterno', '$strNombre', '$strTelefono', '$strCelular', '$strEmail', '$strObservaciones', '$intEstatusID', '".$this->session->userdata('usuario_id')."@', 'INSERT') ");
		*/
	...
	public function autocomplete($q, $limit){
		$row_set = array();
		$this->db->select(" DISTINCT RG.registro_general_id,CONCAT_WS(' ','G -',RG.nombre,RG.ape_paterno,RG.ape_materno) AS persona",FALSE);
