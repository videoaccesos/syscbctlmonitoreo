...
class Residencias_Model extends CI_model
{
   ...
   public function autocompleteNroCasa($q, $limit,$intPrivadaID){
        ...
        $this->db->where('P.estatus_id',1);//Activo
    ...
    public function autocompleteResidente($q, $limit,$intPrivadaID){
	    ...
	    if($query->num_rows > 0){
	      foreach ($query->result_array() as $row){
	        $new_row['value'] = $row['nro_casa'].", ".$row['calle'].", Priv.".$row['privada'].", ".$row['residente']; //Valor a Mostrar en lista
	        $new_row['data'] = $row['residencia_id']; //Valor del ID
	        $row_set[] = $new_row; //build an array
	      }
	    }else {
	...
	public function filtro($strBusqueda, $strPrivada, $intNumRows, $intPos){
		...
		$this->db->select("R.residencia_id,R.nro_casa, R.calle, R.telefono1,R.interfon, 
							(   CASE R.estatus_id
	...
	public function guardar($intPrivadaID,$strNroCasa,$strCalle,$strTelefono1,$strTelefono2,$strInterfon,$strTelInterfon,$strObservaciones,$intEstatusID){
		$datos = array(
...
'telefono_interfon' => $strTelInterfon,
	...
	public function modificar($intResidenciaID,$intPrivadaID,$strNroCasa,$strCalle,$strTelefono1,$strTelefono2,$strInterfon,$strTelInterfon,$strObservaciones,$intEstatusID){
		$datos = array(
			'privada_id' => $intPrivadaID, 
			'nro_casa' => $strNroCasa,
			'calle' => $strCalle,
