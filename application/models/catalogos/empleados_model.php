...
class Empleados_Model extends CI_model
{
	public function guardar($strNombre,$strApePaterno,$strApeMaterno,$strNroSeguroSocial,$intPuestoID,$strNroOperador,$strCalle,$strNroCasa,$strSexo,
							$strColonia,$strTelefono,$strCelular,$strEmail,$strFechaIngreso,$strFechaBaja,$strMotivoBaja,$intEstatusID,$strGoogleKey,$intPermisoAdministrador,$intPermisoEncargadoAdministracion,$intSupervisor){
		$datos = array(
			'nombre' => $strNombre, 
			...
			'google_auth_code' => $strGoogleKey,
			'permiso_administrador' => $intPermisoAdministrador,
			'permiso_encargado_administracion' => $intPermisoEncargadoAdministracion,
			'permiso_supervisor' => $intSupervisor);
	...
	public function autocomplete($q, $limit){
		...
		public function autocomplete_nombre($q, $limit){
		$row_set = array();
		$this->db->select("E.id, E.nombre AS nombre",FALSE);
		$this->db->from('dicc_nombres AS E');
		$this->db->where("E.nombre LIKE  '$q%' ");  
		$this->db->order_by('E.nombre','asc');
		$this->db->limit($limit,0);
	    $query = $this->db->get();
	    if($query->num_rows > 0){
	      foreach ($query->result_array() as $row){
	        $new_row['value'] = $row['nombre']; //Valor a Mostrar en lista
	        $new_row['data'] = $row['id']; //Valor del ID
	        $row_set[] = $new_row; //build an array
	      }
	    }
	    return $row_set; //format the array into json data
		...
		public function autocomplete_apellido($q, $limit){
		$row_set = array();
		$this->db->select("E.id, E.apellido AS apellido",FALSE);
		$this->db->from('dicc_apellidos AS E');
		$this->db->where("E.apellido LIKE  '$q%' ");  
		$this->db->order_by('E.apellido','asc');
		$this->db->limit($limit,0);
	    $query = $this->db->get();
	    if($query->num_rows > 0){
	      foreach ($query->result_array() as $row){
	        $new_row['value'] = $row['apellido']; //Valor a Mostrar en lista
	        $new_row['data'] = $row['id']; //Valor del ID
	        $row_set[] = $new_row; //build an array
	      }
	    }
