...
$this->db->from('tarjetas');
		...
		$this->db->like('lectura', $strBusqueda);
		$this->db->like('estatus_id', 4);
		$res["total_consignacion"] = $this->db->count_all_results();
		...
		$this->db->like('lectura', $strBusqueda);
		$this->db->like('estatus_id', 5);
		$res["total_bajas"] = $this->db->count_all_results();
...
public function autocompleteMC($q, $limit){
			$row_set = array();
			$this->db->select("T.tarjeta_id, T.lectura, T.estatus_id, FV.privada_id");
			$this->db->from('tarjetas as T inner join folios_ventas_consignacion as FV on T.lectura=FV.tarjeta1 or T.lectura=FV.tarjeta2 or T.lectura=FV.tarjeta3 or T.lectura=FV.tarjeta4 or T.lectura=FV.tarjeta5 or T.lectura=FV.tarjeta6 or T.lectura=FV.tarjeta7 or T.lectura=FV.tarjeta8 or T.lectura=FV.tarjeta9 or T.lectura=FV.tarjeta10 ');
			$this->db->where('T.estatus_id', 4);
$this->db->where('FV.privada_id', 22);
			$this->db->where("T.lectura LIKE '%$q%'");  
			$this->db->order_by('T.lectura','asc');
			$this->db->limit($limit,0);
		    $query = $this->db->get();
		    if($query->num_rows > 0){
		      foreach ($query->result_array() as $row){
