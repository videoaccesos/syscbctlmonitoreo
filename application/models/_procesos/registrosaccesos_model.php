...
class RegistrosAccesos_Model extends CI_model
{
	...
	public function enviar_notificaciones_emails($id){
		$result = null;
		//Datos del Acceso
		$this->db->select("DATE_FORMAT( DATE_ADD(RA.fecha_modificacion, INTERVAL -2 HOUR ),'%d-%m-%Y') AS fecha, DATE_FORMAT( DATE_ADD(RA.fecha_modificacion, INTERVAL -2 HOUR ),'%r') AS hora,P.descripcion AS privada, R.nro_casa, R.calle, 
	...
	public function filtro_grafica($strFechaHoraInicio,$strFechaHoraFin,$intPrivadaID,$intResidenciaID,$strSolicitanteID,
		...
		if($intFiltro == 0)
			$this->db->select("DATE_FORMAT( DATE_ADD(RA.fecha_modificacion, INTERVAL -1 HOUR ),'%Y-%m-%d %H:00:00') AS fecha, count(*) AS total", FALSE);
		else if($intFiltro == 1)
			$this->db->select("DATE_FORMAT( DATE_ADD(RA.fecha_modificacion, INTERVAL -1 HOUR ),'%Y-%m-%d') AS fecha, count(*) AS total", FALSE);
		else
			$this->db->select("DATE_FORMAT( DATE_ADD(RA.fecha_modificacion, INTERVAL -1 HOUR ),'%Y-%m-01') AS fecha, count(*) AS total", FALSE);
		$this->db->from('registros_accesos RA');
		//$this->db->join('privadas P','P.privada_id = RA.privada_id','inner');
		//$this->db->join('residencias R','R.residencia_id = RA.residencia_id','inner');
		//$this->db->join('empleados E','E.empleado_id = RA.empleado_id','inner');
