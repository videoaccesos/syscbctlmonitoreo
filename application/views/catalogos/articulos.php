	<form id="frmSearch" action="#" method="post" onsubmit="return(false)">
			<div style="display:inline-block;">
	          <label for="txtBusqueda"><h5>Descripción</h5></label>
	          <div class="input-append">
	              <input id ="txtBusqueda" name="strBusqueda" value="" type="text" placeholder="Teclea Descripción" tabindex="-4">
	              <div class="btn-group" >
	...
	<table class="table table-stripped" id="dgArticulos">
		<thead align="center">
			<tr>
        <th width="25px">Código</th>
			 <th width="20px">Modelo</th>
			 <th>Descripción</th>
       <th width="20px">Costo</th>
       <th width="20px">Existencia</th>
			 <th width="20px">Estado</th>
			 <th width="15px">A/B</th>
        ...
        <script id="plantilla_articulos" type="text/template">
          {{#rows}}
          <tr><td>{{articulo_id}}</td>
          <td>{{modelo}}</td>
				<td>{{descripcion}}</td>
				<td>{{costo}}</td>
        <td>{{existencia}}</td>
        <td>{{estatus}}</td>
				<td><a onclick="fnCambiarEstado({{articulo_id}},{{estatus_id}})" class="btn btn-mini"><i class="icon icon-eye-open"></i></a></td>
				<td><a onclick="fnEditar({{articulo_id}})" class="btn btn-mini"><i class="icon icon-pencil"></i></a></td>
                <td><a onclick="fnEliminar({{articulo_id}})" class="btn btn-mini"><i class="icon icon-trash"></i></a></td>
            </tr>
          {{/rows}}
          {{^rows}}
          <tr> 
	...
	<div id="myModal" class="modal hide fade" tabindex="0" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-header">
			...
			<h3 id="myModalLabel">Registro de Artículo</h3>
		</div>
		<div class="modal-body">
			<form name="frmArticulos" id="frmArticulos" action="#" method="post" onsubmit="return(false)" autocomplete="off">
				<input id="txtArticuloID" type="hidden" value="">
				<label for="txtModelo"><h5>Modelo</h5></label>
				<input style="width:105px;" id="txtModelo" type="text" placeholder="Ingresa Modelo" tabindex="2">
        ...
        <div style="display:inline-block; padding-right:115px;">
          <label for="txtExistencia"><h5>Existencia</h5></label>
          <div class="input-prepend">
             <input style="width:80px;text-align:right;" id="txtExistencia" type="text" value="" tabindex="5">
          </div>
        </div>
        <div style="display:inline-block;">
...
//---------- Funciones para la Busqueda
  ...
  function paginacion(){
    ...
    $.post('catalogos/articulos/paginacion',
                 {strBusqueda:$('#txtBusqueda').val(), intPagina:pagina},
                  function(data) {
                    $('#dgArticulos tbody').empty();
                    var temp = Mustache.render($('#plantilla_articulos').html(),data);
                    $('#dgArticulos tbody').html(temp);
...
//--------- Funciones para el Modal
  ...
  function fnBuscar(item){
    if (item.data) {
      $(location).attr('href','catalogos/articulos/editar/'+item.data[0]);     
    }
    else
      $(location).attr('href','catalogos/articulos');        
  }
  function fnValidar(){
    return true;
  }
  function fnGuardar(){
    if(fnValidar()) {
      $.post('catalogos/articulos/guardar',
                  { intArticuloID: $('#txtArticuloID').val(), 
                  	strModelo: $('#txtModelo').val(),
                    strDescripcion: $('#txtDescripcion').val(),
                    dblCosto: $('#txtCosto').val(),
                    intExistencia: $('#txtExistencia').val(),
  ...
  function fnCambiarEstado(id,estado){
      $.post('catalogos/articulos/cambiar_estado',
                  { intArticuloID: id,
                    intEstatusID: estado
                  },
                  function(data) {
  ...
  function fnEditar(id){
    $.post('catalogos/articulos/editar',
                  { intArticuloID:id
                  },
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
                    if(data.row){
                      $('#txtArticuloID').val(data.row.articulo_id);
                      $('#txtModelo').val(data.row.modelo);
                      $('#txtDescripcion').val(data.row.descripcion);
                      $('#txtCosto').val(data.row.costo);
                      $('#txtExistencia').val(data.row.existencia);
   ...
   function fnEliminar(id){
    $.post('catalogos/articulos/eliminar',
                  { intArticuloID: id
                  },
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
