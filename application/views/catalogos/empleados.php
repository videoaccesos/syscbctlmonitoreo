  ...
  <div class="btn-group" style="display:inline-block;float:right;padding-top:40px;">
    ...
    <div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="top:7%; width:985px; left:38%;" >
        ...
        <div class="modal-body" style="max-height:100%; height:100%;">
                ...
                <div style="display:inline-block; vertical-align: top;">
                    ...
                    <div>
                        <div>
                          ...
                          <textarea id="txtMotivo" class="span6" rows="4" placeholder="Ingrese Motivo" style="resize:none;"></textarea>
                        </div>
                    </div>
                    <div>
                        <div style="display:inline-block;">
                            <label for="cmbAdministradores">Permiso 1</label>
                            <select class="span3" id="cmbAdministradores">
                            </select>
                        </div>
                        <div style="display:inline-block;">
                            <label for="cmbEncargadosAdmon">Permiso 2</label>
                            <select class="span3" id="cmbEncargadosAdmon">
                            </select>
                        </div>
                    </div>
                    <div>
                        <div style="display:inline-block;">
                            <label for="cmbSupervisores">Permiso 3</label>
                            <select class="span3" id="cmbSupervisores">
                    ...
                    <div>
                        <div style="display:inline-block;">
                            <label for="txtGoogleKey">Clave Secreta Google</label>
                            <input class="span3" id="txtGoogleKey" type="text" tabindex="99" maxlength="4" disabled>
                        </div>
                        <div style="display:inline-block; margin-bottom:5px">
                            <button class="btn btn-primary" id="btnGenerarClaveGoogle" style="margin-bottom: 10px;"><i class="icon-refresh icon-white"></i></button>
                        </div>
                    </div>
                </div>
          ...
          <script id="plantilla_administradores" type="text/template"> 
            {{#administradores}}
              <option value="{{value}}">{{nombre}} {{ape_paterno}} {{ape_materno}} </option>
            {{/administradores}} 
          </script>
          
          <script id="plantilla_encargadosadmon" type="text/template"> 
            {{#encargadosadmon}}
              <option value="{{value}}">{{nombre}} {{ape_paterno}} {{ape_materno}}</option>
            {{/encargadosadmon}} 
          </script>
          
          <script id="plantilla_supervisores" type="text/template"> 
            {{#supervisores}}
              <option value="{{value}}">{{nombre}} {{ape_paterno}} {{ape_materno}}</option>
            {{/supervisores}} 
          </script>
        ...
        <div class="modal-footer">
            ...
            <button class="btn" tabindex="-1" style="float:left;" onclick="fnShowBitacora('#txtEmpleadoID','empleados');"><i class="icon-calendar"></i> Historial</button>
        </div>
  </div>
...
//---------- Funciones para la Busqueda
  ...
  function cmbAdministradores_set() {
    $.post('catalogos/puestos/administradores',
                  {},
                  function(data) {
                    $('#cmbAdministradores').empty();
                    var temp = Mustache.render($('#plantilla_administradores').html(),data);
                    $('#cmbAdministradores').html(temp);
                  }
                  ,'json');
  }
  
  function cmbEncargadosAdmon_set() {
    $.post('catalogos/puestos/encargadosadmon',
                  {},
                  function(data) {
                    $('#cmbEncargadosAdmon').empty();
                    var temp = Mustache.render($('#plantilla_encargadosadmon').html(),data);
                    $('#cmbEncargadosAdmon').html(temp);
                  }
                  ,'json');
  }
  
  function cmbSupervisores_set() {
    $.post('catalogos/puestos/supervisores',
                  {},
                  function(data) {
                    $('#cmbSupervisores').empty();
                    var temp = Mustache.render($('#plantilla_supervisores').html(),data);
                    $('#cmbSupervisores').html(temp);
                  }
                  ,'json');
  }
  function generarCodigoAuth() {
        var data = "";
		$.post('../google/generarCodigoAuth.php', data, function(data,status){
			console.log("submitnig result ====> Data: " + data + "\nStatus: " + status);
			if( data != ""){
				$('#txtGoogleKey').val(data);
			}
			else{
				alert("No se pudo generar el código de autenticación.");
			}
			
		});
  }
  function fnGenerarClaveGoogle() {
        var data = "";
		$.post('../google/generarCodigoAuth.php', data, function(data,status){
			console.log("submitnig result ====> Data: " + data + "\nStatus: " + status);
			if( data != ""){
    ...
    if(fnValidar()) {
      $.post('catalogos/empleados/guardar',
                  { intEmpleadoID: $('#txtEmpleadoID').val(), 
                    ...
                    strGoogleKey: $('#txtGoogleKey').val(),
                    intPermisoAdministrador: $('#cmbAdministradores').val(),
                    intPermisoEncargadoAdministracion: $('#cmbEncargadosAdmon').val(),
                    intSupervisor: $('#cmbSupervisores').val()
                  },
                  function(data) {
                    if(data.resultado){
                      fnNuevo();
  ...
  function fnEditar(id){
    $.post('catalogos/empleados/editar',
                  ...
                  if(data.row){
                      ...
                      $('#txtGoogleKey').val(data.row.google_auth_code);
                      $('#cmbAdministradores').val(data.row.permiso_administrador.toString());
                      $('#cmbEncargadosAdmon').val(data.row.permiso_encargado_administracion.toString());
                      $('#cmbSupervisores').val(data.row.permiso_supervisor.toString());
...
//---- Script para el Modal
     ...
     $("#btnGenerarClaveGoogle").click(fnGenerarClaveGoogle);
...
//---- Codigo Inicial para el Primer form
     ...
     paginacion();
     cmbPuestoID_set();
     cmbAdministradores_set();
     cmbEncargadosAdmon_set();
     cmbSupervisores_set();
     generarCodigoAuth();
  });
</script> 