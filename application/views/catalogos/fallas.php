	...
	<table class="table table-stripped" id="dgFallas">
		<thead>
			<tr>
        ...
        <script id="plantilla_fallas" type="text/template">
          {{#rows}}
          <tr><td>{{codigo}}</td>
				<td>{{descripcion}}</td>
				<td>{{estatus}}</td>
				<td><a onclick="fnCambiarEstado({{falla_id}},{{estatus_id}})" class="btn btn-mini"><i class="icon icon-eye-open"></i></a></td>
				<td><a onclick="fnEditar({{falla_id}})" class="btn btn-mini"><i class="icon icon-pencil"></i></a></td>
                <td><a onclick="fnEliminar({{falla_id}})" class="btn btn-mini"><i class="icon icon-trash"></i></a></td>
            </tr>
          {{/rows}}
          {{^rows}}
          <tr> 
	...
	<div id="myModal" class="modal hide fade" tabindex="0" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-header">
			...
			<h3 id="myModalLabel">Falla</h3>
		</div>
		<div class="modal-body">
			<form name="frmFallas" id="frmFallas" action="#" method="post" onsubmit="return(false)" autocomplete="off">
				<input id="txtFallaID" type="hidden" value="">
...
//---------- Funciones para la Busqueda
  ...
  function paginacion(){
    ...
    $.post('catalogos/fallas/paginacion',
                 {strBusqueda:$('#txtBusqueda').val(), intPagina:pagina},
                  function(data) {
                    $('#dgFallas tbody').empty();
                    var temp = Mustache.render($('#plantilla_fallas').html(),data);
                    $('#dgFallas tbody').html(temp);
...
//--------- Funciones para el Modal
  ...
  function fnBuscar(item){
    if (item.data) {
      $(location).attr('href','catalogos/fallas/editar/'+item.data[0]);     
    }
    else
      $(location).attr('href','catalogos/fallas');        
  }
  function fnValidar(){
    return true;
  }
  function fnGuardar(){
    if(fnValidar()) {
      $.post('catalogos/fallas/guardar',
                  { intFallaID: $('#txtFallaID').val(), 
  ...
  function fnCambiarEstado(id,estado){
      $.post('catalogos/fallas/cambiar_estado',
                  { intFallaID: id,
                    intEstatusID: estado
                  },
                  function(data) {
  ...
  function fnEditar(id){
    $.post('catalogos/fallas/editar',
                  { intFallaID:id
                  },
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
                    if(data.row){
                      $('#txtFallaID').val(data.row.falla_id);
   ...
   function fnEliminar(id){
    $.post('catalogos/fallas/eliminar',
                  { intFallaID:id
                  },
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
