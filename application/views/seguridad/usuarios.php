 ...
 <!-- Modal - FrmUsuarios -->
    <div id="myModal" class="modal hide fade" tabindex="0" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"  >
            ...
            <form name="frmUsuarios" id="frmUsuarios" action="#" method="post" onsubmit="return(false)" autocomplete="off">
              <div style="display:-webkit-inline-box;">
                  <div style="width:240px;">
                    <input type="hidden" id="txtUsuarioID2" name="intUsuarioID" value="">
                    <label for="txtUsuario2">Usuario</label>      
                    <input class="span3" id="txtUsuario2" name="strUsuario" type="text" value="" tabindex="2" placeholder="Teclea el Usuario" maxlength="20">
        ...
        <div class="modal-footer">
            ...
            <button class="btn" tabindex="-1" style="float:left;" onclick="fnShowBitacora('#txtUsuarioID2','usuarios');"><i class="icon-calendar"></i> Historial</button>
        </div>
  </div>     
  ...
  function fnGuardar(){
    if(fnValidar()) {
      $.post('seguridad/usuarios/guardar',
                  { intUsuarioID: $('#txtUsuarioID2').val(),
                    strUsuario: $('#txtUsuario2').val(),
  ...
  function fnEditar(id){
    $.post('seguridad/usuarios/editar',
                  ...
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
                    if(data.row){
                      $('#txtUsuarioID2').val(data.row.usuario_id);
                      $('#txtUsuario2').val(data.row.usuario);
                      $('#txtPassword').val();
                      $('#txtConfirmar').val();
