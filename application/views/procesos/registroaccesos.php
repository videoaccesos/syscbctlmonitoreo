...
<input type="hidden" id="txtUsuarioID" value="<?php echo  strtoupper($this->session->userdata('usuario_id')); ?>">
  ...
  <!-- Campo oculto que se utiliza para asignar el tipo de tarjeta del dns seleccionado -->
  <input id="txtTipoTarjetaDNS" type="hidden" value="">
  <!-- Campo oculto que se utiliza para asignar la contraseña del tipo de tarjeta 2 del dns seleccionado -->
  <input id="txtContrasenaDNS" type="hidden" value="">

	<div style="display:inline-block; font-size:12px; padding-top:5px; float:right;">
    &Uacute;ltima hora - Total Llamadas: <label id="lblTotalLlamadas" style="display:inline-block;font-size:14px; font-weight:bold;">0</label>
    ...
    <div>
	  	<div>
			<a id="btnEnviarAtencionClientes" href="" type="button" class="btn btn-primary" data-toggle="modal" onclick="fnGuardarQuejaSugerencia()"><i class="icon-arrow-right icon-white"></i> Enviar a Atención a Clientes</a>
                
	  	</div>
    </div>
    <div>
	  	<div>
			<div class="input-append datetimepicker" style="visibility:hidden;">
                  <input id="txtFechaHora" class="span2" data-format="dd-MM-yyyy hh:mm" type="text" tabindex="8">
                  <span class="add-on">
	...
	<div style="display:inline-block; vertical-align: top; margin-top:15px;" id="testvideo">
  ...
  <div> 
	...
	<table class="table table-stripped" id="dgPrivadasMonitoristas">
		<thead>
			<tr>
			 <th width="60px">Privada</th>
			 <!--<th style="text-align:center;" width="20px">Estado</th>-->
			 <th style="text-align:center;" width="20px">Revisado</th>
			 <th style="text-align:center;" width="20px">Comentarios</th>
			</tr>
		</thead>
		<tbody>
		</tbody>
        <script id="plantila_privadasMonitoristas" type="text/template">
          {{#rows}}
          <tr>
          <td>{{descripcion}}</td>
				<td style="text-align:center;"><a onclick="fnCambiarEstado({{consecutivo_id}},{{estatus_id}})" class="btn btn-mini"><i class="icon icon-check"></i></a></td>
				<td style="text-align:center;"><a onclick="fnComentarios()" class="btn btn-mini"><i class="icon icon-comment"></i></a></td>
            </tr>
          {{/rows}}
          {{^rows}}
          <tr> 
 ...
 <script id="plantilla_DNS" type="text/template"> 
  {{#dns}}
    <option id="opDNS1" value="{{dns1_id}}|{{dns1}}|{{puerto1}}|{{tipo_tarjeta1}}|{{contrasena_1}}">{{alias1}}</option>
    <option id="opDNS2" value="{{dns2_id}}|{{dns2}}|{{puerto2}}|{{tipo_tarjeta2}}|{{contrasena_2}}">{{alias2}}</option>
    <option id="opDNS3" value="{{dns3_id}}|{{dns3}}|{{puerto3}}|{{tipo_tarjeta3}}|{{contrasena_3}}">{{alias3}}</option>
  {{/dns}} 
</script>
...
<script id="plantilla_visitantes" type="text/template">
     ...
     <!-- Modal -->
	<div id="myModalComentarios" class="modal hide fade" tabindex="0" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true" tabindex="1">x</button>
			<h3 id="myModalLabel">Comentarios</h3>
		</div>
		<div class="modal-body">
			<form name="frmComentarios" id="frmComentarios" action="#" method="post" onsubmit="return(false)" autocomplete="off">
				<label for="txtDescripcion"><h5>Comentarios</h5></label>
				<input class="span6" id="txtComentarios" type="text" placeholder="Comentarios" tabindex="1">
			</form>
		</div>
	</div>
<!-- Modal -->
			...
			<div style="display:inline-block;">
				<div style="max-width:470px; widht:90%; display:inline-block; padding-right:17px;">     
						<div style="display:inline-block; padding-right:17px;">
							...
							<input class="span2" id="txtCelular" type="text" placeholder="Celular" maxlength="14" tabindex="25" style="display: inline-block; margin-right:15px;">
              <input class="span2" id="txtFechanac" type="text" placeholder="Fecha Nacimiento" maxlength="14" tabindex="26" style="display: inline-block;">
...
//http://www.comolohago.cl/como-anadir-campos-a-un-formulario-dinamicamente/
  ...
  function fnCambiarEstado(id,estado){
      $.post('catalogos/monitoristas/cambiar_estado',
                  { intConsecutivoID: id,
                    strComentarios: $('#txtComentarios').val(),
                    intEstatusID: estado
                  },
                  function(data) {
                    if(data.resultado){
...
requestMethod.call(element);
    ...
    $('#txtURLDNS').val('');
    $('#txtDNSID').val('');
    $('#txtTipoTarjetaDNS').val('');
    $('#txtContrasenaDNS').val('');
  ...
  function fnHabilitarFormulario(bolBlok){
    if(bolBlok){
      ...
      $('#txtQuejasSugerencias').removeAttr('disabled');
      $('#btnEnviarAtencionClientes').removeAttr('disabled');
      ...
      $('#txtQuejasSugerencias').attr('disabled','disabled');
      $('#btnEnviarAtencionClientes').attr('disabled','disabled');
    }
  ...
  function fnGuardarQuejaSugerencia(){
    if(fnValidar()){
    var fechaHora = $('#txtFechaHora').val();
      $.post('procesos/ordenesservicio/guardar',
                { intOrdenServicioID: '',
                  ...
                  intPrivadaID: $('#cmbPrivadaID').val(),
                  intTecnicoID: 264,
                  strFechaAsistio: fechaHora,
                  intTiempo: 0,
                  intCodigoServicioID: 46,
                  strDetalleServicio: "Datos de Residente y Residencia:\n"+$('#txtResidencia').val()+"\nQueja/Sugerencia:\n"+$('#txtQuejasSugerencias').val(),
                  intDiagnosticoID: 89,
                  strDetalleDiagnostico: ' '
                },
                function(data) {
                  if(data.resultado){
                    alert("La Orden de Servicio para Atención a Clientes se ha Generado Correctamente!");
                    $('#txtQuejasSugerencias').val("");
                  }
                  $('#divMensajes').html(data.mensajes);
                  bolGuardando = false;
 ...
 //Función que se utiliza para mostrar el video seleccionado
     ...
     $.post('catalogos/residencias/info',
                  ...
                  function(data) {
                    if(data){
                      ...
                      strPop += "<input type='button' value='Cerrar' onclick='cerrarPopUpResidencia();' style='float:right;' />";
                      strPop += "</br>";
  ...
  function cerrarPopUpResidencia(){
    $('#txtResidenciaPop').popover('hide');
  }
  ...
  function load_info_Solicitante(item){
     $.post('procesos/registroaccesos/info_Solicitante',
                  ...
                  function(data) {
                    if(data != null){
                      ...
                      if(data.observaciones != "")
                        strPop += "<b>Notas: </b>" + data.observaciones +"<br>";
                      strPop += "<input type='button' value='Cerrar' onclick='cerrarPopUpSolicitante();' style='float:right;' />";
                      strPop += "</br>";
    ...
    function cerrarPopUpSolicitante(){
    $('#txtSolicitantePop').popover('hide');
  }
  ...
  function fnEditar_RegistroGeneral(id){
    $.post('catalogos/registrosgenerales/editar',
                  ...
                  if(data.row){
                      ...
                      $('#txtFechanac').val(data.row.fechanac);
      ...
      //Separar la cadena 
       ...
       for (var intCont=0; intCont < strRenglones.length; intCont++) {
          ...
          //el combobox
           ...
           //Dependiendo del estado actual del relays cambiar las opciones a mostrar en 
           //el combobox
           var strOpcionesEstado='';
           //Si la tarjeta seleccionada es 2
           if($("#txtTipoTarjetaDNS").val()=='Tarjeta 2')
           {
               //Regresar el estado actual del relay
               if(strEstados[intCont]=='Activo')
               {
                  //Se cambiara la opción del estado a mostrar 
                  strOpcionesEstado='<option value=0>Inactivo</option></select>';

               }
               else
               {
                   //Se cambiara la opción del estado a mostrar 
                   strOpcionesEstado='<option value=1>Activo</option></select>';
               }

           }
           else//Si la tarjeta del DNS es la 1
           {
               //Asignar el estado como Activo debido a que la tarjeta no envia respueta del estado 
               //actual del relays
                strEstados[intCont]='Activo';
               //Se asignara la opción del estado a mostrar 
                strOpcionesEstado='<option value=0>Inactivo</option></select>';
           ...
           //Agregar campos en la tabla
            ...
            else
            {
              ...
              NvoCampo.innerHTML= 
                 ...
                 "          <option value='"+fnConvertirEstadoRelays(strEstados[intCont])+"'>" +strEstados[intCont]+ "</option>" +
                            strOpcionesEstado+
                 "     </td>" +
                 "   </tr>" +
                 "</table>";
 ...
 //Función que se utiliza para cambiar el estado recuperado del relays por el valor
 //de activación Activo-1 e Inactivo-0
  function fnConvertirEstadoRelays(strEstado)
  {
      var intEstado=0;
      //Si el estado del relays es Activo
      if(strEstado=="Activo")
      {
        //Asignar el valor de 1
        intEstado=1;
      } 
      //Regresar el valor del estado
      return intEstado;

  } 

 //Función que se utiliza para abrir ventanas emergentes por relays del concepto seleccionado
 //dependiendo del tipo de tarjeta (DNS)
 function fnAbrirVentanaRelay(intRenglon,intIndice){
       //Si la tarjeta del DNS es la 2
       if($("#txtTipoTarjetaDNS").val()=='Tarjeta 2')
       {
              //Hacer un llamado a la dunción para activar relays 
              //de la tarjeta 2
              fnAbrirVentanaRelayTarjeta2(intRenglon,intIndice);
       }
       else //Tarjeta 1
       {
            //Hacer un llamado a la dunción para activar relays
            //de la tarjeta 1
            fnAbrirVentanaRelayTarjeta1(intRenglon,intIndice);
       }
 }

 //Función que se utiliza para abrir ventanas emergentes por relays del concepto seleccionado
 //cuando la tarjeta del DNS sea la 2
 function fnAbrirVentanaRelayTarjeta2(intRenglon,intIndice){
       var strRelaysConceptoT2='';
        var strRelaysSelT2='';
      ...
      var intNumeroRelaysT2=0;
      //Verificar si la cadena tiene caracter |,y así poder separar la cadena
      if(fnBuscaCarater(strRelaysConceptoT2)==false){
        intNumeroRelaysT2=1;
        
      }
      else
      {
        //Separa la cadena quita '|' (por cada relays)  ejemplo:RELAY1|RELAY2|RELAY3...
        strRelaysConceptoT2 = strRelaysConceptoT2.split('|');
        //Asignar el número de relays (ejemplo:RELAY8|RELAY9 son 2)
        intNumeroRelaysT2=strRelaysConceptoT2.length;
    ...
    //Cadena concatenada con el id del boton y el indice seleccionado
      ...
      var strRelaysActivar='';
      var strUltimoRelays='';

     //Formar cadena para cambiar el estado de los relays en caso de que sea mayor que uno 
     if(intNumeroRelaysT2>1)
     {
        //Variable que se utiliza como contador para la activacion de varios relays
         var intContRelay=0;
        
         //Mientras contador inicial sea menor al número de relays
         while(intContRelay<intNumeroRelaysT2)
         {
            strUltimoRelays=strRelaysConceptoT2[intContRelay];
            //Concatenar los datos del relays para formar cadena de relays a activar 
            strRelaysActivar=strRelaysActivar+'&'+strRelaysConceptoT2[intContRelay]+'='+intEstadoRelay; 
            //Incrementar contador para pasar al siguiente relays
            intContRelay++;
     ...
     {
        //Concatenar los datos del relays para formar cadena de relays a activar 
        strRelaysActivar='&'+strRelaysConceptoT2+'='+intEstadoRelay;
     }

    //Reemplazar cadena para obtener el indice del relays, de esta forma se recuperara el 
    //estado en que se encuentran los relays despues del cambio (desde el servidor)
    strUltimoRelays=strUltimoRelays.replace("Relay", "");
     
    //Url del servidor 
    var URLServidor=$("#txtURLDNS").val()+"/current_state.xml?pw="+$("#txtContrasenaDNS").val();
    ...
    var  strUrl=URLServidor+strRelaysActivar;
    //Si el tiempo de activación es indefinido
    if(strTiempoActivacion=='')
    {
        //var url="http://videoaccesos5.sytes.net:9999?relay="+id+";st="+intEstadoRelay+"";
        //ventana_secundaria =eval('ventana'+ intRenglon + "=window.open(strUrl,'ventana'+intRenglon,'width=120px,height=80px,top=1800px,left=1500px,location=no,resizable=no')")
        
        //FUNCION PARA HACER EL POSTEO DE LAS PETICIONES GET A LAS TARJETAS DE RELAY
        ...
        http.onreadystatechange = function(){
            if(http.readyState == 4){    
                ...
                $("#divMensajes").html("Relay Activado exitosamente..");
            }else{
                $("#divMensajes").html("Existe un problema con la tarjeta de relays..");
            }
        }
        http.send();
        ...
        //setTimeout( function() {ventana_secundaria.close();
        //      $.ajax('catalogos/relaysactivacion/guardar',{
        //                                "type" : "post",
        //                                "data": {intPrivadaID:$("#cmbPrivadaID").val(),
        //                                         intDNSID: $("#txtDNSID").val(),
        //                                         intRenglonID: intRenglon,
        //                                         strRelays:strRelaysSelT2,
        //                                         strTiempo:strValor,
        //                                         strTipoTarjeta:$("#txtTipoTarjetaDNS").val(),
        //                                         strURLTarjeta2:URLServidor,
        //                                         strContrasenaTarjeta2:$("#txtContrasenaDNS").val(),
        //                                         intIndiceRelayTarjeta2:strUltimoRelays
    ...
    else
    {
          ...
          //fin de seguir con el proceso cuando se cambie de privada)
            var strDatos = $('#cmbPrivadaID').val()+'|'+$("#cmbPrivadaID option:selected").html()+'|'+$('#txtResidenciaID').val()+'|'+$('#txtResidencia').val()+'|'+
                         $('#txtSolicitanteID').val()+'|'+ $('#txtSolicitante').val()+'|'+$('#txtOperadorID').val()+'|'+$('#txtOperador').val()+'|'+$('#cmbDNSID option:selected').html()
                         +'|'+$('#txtDNSID').val()+'|'+$('#txtTipoTarjetaDNS').val()+'|'+strRelaysSelT2+'|'+URLServidor+'|'+$("#txtContrasenaDNS").val()
                         +'|'+intNumeroRelaysT2+'|'+strRelaysConceptoT2;
                    
           var strDatosDNS=$('#cmbDNSID').val();
          ...
          strUrlPrueba="procesos/registroaccesosRelays?strDatos="+strDatos+"&strDatosDNS="+strDatosDNS+"&strDatosRelays="+strDatosRelays+"&strVentanaPrincipal="+strVentanaPrincipal;
  ...
  //Función que se utiliza para abrir ventanas emergentes por relays del concepto seleccionado
 //cuando la tarjeta del DNS sea la 1
  function fnAbrirVentanaRelayTarjeta1(intRenglon,intIndice){
      ...
      if(strTiempoActivacion!='')
      {
         //Abrir pagina registroaccesosRelays para continuar con el proceso de activación del relays seleccionado
          ...
          //fin de seguir con el proceso cuando se cambie de privada)
            var strDatos = $('#cmbPrivadaID').val()+'|'+$("#cmbPrivadaID option:selected").html()+'|'+$('#txtResidenciaID').val()+'|'+$('#txtResidencia').val()+'|'+
                         $('#txtSolicitanteID').val()+'|'+ $('#txtSolicitante').val()+'|'+$('#txtOperadorID').val()+'|'+$('#txtOperador').val()+'|'+$('#cmbDNSID option:selected').html()
                         +'|'+$('#txtDNSID').val()+'|'+$('#txtTipoTarjetaDNS').val();
...
paginacionTotalLlamadas();
  	    ...
  	    //EJECUTA EL AUTOCOMPLETE DE NOMBRE
      	$("#txtNombre").autocomplete("catalogos/empleados/autocomplete_nombre", 
         { minChars:1,matchSubset:1,matchContains:1,cacheLength:6,onItemSelect:null,selectOnly:0,remoteDataType:"json"} 
          );

  	    //EJECUTA EL AUTOCOMPLETE DE APELLIDO
      	$("#txtApePaterno").autocomplete("catalogos/empleados/autocomplete_apellido", 
         { minChars:1,matchSubset:1,matchContains:1,cacheLength:6,onItemSelect:null,selectOnly:0,remoteDataType:"json"} 
          );

  	    //EJECUTA EL AUTOCOMPLETE DE APELLIDO
      	$("#txtApeMaterno").autocomplete("catalogos/empleados/autocomplete_apellido", 
         { minChars:1,matchSubset:1,matchContains:1,cacheLength:6,onItemSelect:null,selectOnly:0,remoteDataType:"json"} 
          );
      ...
      $("#btnActivacionRelays").click(function(event){
       ...
       //Si el id de la privada es diferente de 0
        ...
        {  
              ...
              //Asignar el tipo de tarjeta del DNS
              $("#txtTipoTarjetaDNS").val(stCadenasDNS[3]);
              //Asignar la contraseña del  DNS
              $("#txtContrasenaDNS").val(stCadenasDNS[4]);

              //FALTA CONCATENAR EL TIPO DE TARJETA Y TRAER LA CONTRASEÑA 
              //SI EL TIPO DE TARJETA ES 2-ACTIVAR RELAYS CON XML Y RECUPERAR CONTRASEÑA
              //SI EL TIPO DE TARJETA ES 1 ACTIVAR RELAYS NORMAL Y REVISAR POR QUE MARCA ERROR CUANDO
              //SON 5M SEG Y 2-MINUTOS ABRE VENTANA DE GESTOR HOSTING
...
//---- Script para el Modal
     ...
     $("#txtApePaterno,#txtApeMaterno,#txtNombre").keyup(paginacion_RegistrosGenerales);
...
/*printWindow = window.open( printContents, "index.html"); 
printWindow.open();
printWindow.print(); 
printWindow.close();*/
     var divText = document.getElementById(divName).innerHTML;
     myWindow=window.open('','','width=380,height=380');
     var doc = myWindow.document;
     var html1 = '<!DOCTYPE HTML><html><head><title>HAI</title></head><body>';
     var html2 = '</body></html>';
     var fullpagefinal = html1.concat(divText,html2); 
     doc.write(fullpagefinal);
    
doc.open(fullpagefinal);
setTimeout(function(){}, 1000);

doc.close(fullpagefinal);
doc.close();
