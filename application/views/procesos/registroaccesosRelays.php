<?php
    ...
    //Separar cadena que contiene los datos Informativos
    ...
    //Definir las siguienetes variables para evitar errores php 
    $intPrivadaID=0;
    $strPrivada='';
    ...
    $strAliasDNS='';
    $strTipoTarjetaDNS='';
    //Asignar valor en caso de que el tipo de tarjeta del DNS sea 2
    $strContrasenaDNS='';
    $URLServidor='';
    $intNumeroRelaysT2=0;
    $strRelaysConceptoT2='';
   ...
   //Datos del solicitante
    ...
    //Datos del Empleado
    $intOperadorID=$arrElementos[6];
    $strOperador=$arrElementos[7];
   //Datos del DNS
    $strAliasDNS=$arrElementos[8];
    $intDNSID=$arrElementos[9];
    $strTipoTarjetaDNS=$arrElementos[10]; 

    //Si el tipo de tarjeta del DNS es 2
    if($strTipoTarjetaDNS=='Tarjeta 2')
    {
        $strRelaysSelT2=$arrElementos[11]; 
        $URLServidor=$arrElementos[12]; 
        $strContrasenaDNS=$arrElementos[13]; 
        $intNumeroRelaysT2=$arrElementos[14]; 
        $strRelaysConceptoT2=$arrElementos[15]; 
    }
...
<form id="frmRegistroAccesoRelays" action="#" method="post" onsubmit="return(false)" autocomplete="off" >
  ...
  <input id="txtDNSID" type="hidden" value="<?php echo $intDNSID; ?>">
    ...
    <div  style="position: absolute; left:430px;margin-top:-10px; width: 350px; z-index:1060;" id="divRelasyDNS2">
        ...
        <div>
              <input id="cmbRADNSID" type="hidden" value="<?php echo $strDatosDNS; ?>">
        ...
        <div id="divActivacionRelays2">
            ...
            <!-- Campo oculto que se utiliza para asignar el tipo de tarjeta correspondiente al DNS seleccionado-->
            <input type="hidden" id="txtTipoTarjetaDNS" value="<?php echo $strTipoTarjetaDNS; ?>">
            <!-- Campo oculto que se utiliza para asignar la contraseña del tipo de tarjeta 2 correspondiente al DNS seleccionado-->
            <input type="hidden" id="txtContrasenaDNS" value="<?php echo $strContrasenaDNS; ?>">
            <!-- Campo oculto que se utiliza para asignar la url del DNS cuando el tipo de tarjeta sea 2-->
            <input type="hidden" id="txtURLServidor" value="<?php echo $URLServidor; ?>">
            <!-- Campo oculto que se utiliza para asignar la cadena con los relays del DNS cuando el tipo de tarjeta sea 2-->
            <input type="hidden" id="txtRelaysSelT2" value="<?php echo $strRelaysSelT2; ?>">
            <!-- Campo oculto que se utiliza para asignar el número de relays del DNS cuando el tipo de tarjeta sea 2-->
            <input type="hidden" id="txtNumeroRelaysT2" value="<?php echo $intNumeroRelaysT2; ?>">
             <!-- Campo oculto que se utiliza para asignar el concepto de los relays separados por |  cuando el tipo de tarjeta sea 2-->
            <input type="hidden" id="txtRelaysConceptoT2" value="<?php echo $strRelaysConceptoT2; ?>">
	...
	<div style=" display:inline-block; padding-right:15px; ">     
		<div>
			<div class="input" style="display:inline-block; padding-right:17px;">
				...
				<input type="hidden" id="txtOperadorID" value ="<?php echo $intOperadorID;?>">
				<input type="text" id="txtOperador" style="width:330px;" tabindex="-1" value="<?php echo $strOperador; ?>" disabled>
			</div>
		</div>
		<div>
...
//--------- Funciones para el Modal Registro General
  function fnNuevo(){
   var intDNSIDC=$('#txtDNSID').val();
    ...
    //Hacer un llamado a la función para mostrar los relays
    fnMostrarRelays();
   //Abrir ventanas emergentes para realizar cambios en el servidor
  ...
  //Función que se utiliza para mostrar los relays del DNS
 ...
 function fnAgregarRelaysDNSR(strDetallesRenglones,strDetallesRelays, strDetallesConceptos,strDetallesEstados,strDetallesTiempos){
           ...
           //Agregar campos en la tabla
            ...
            else
            {
              ...
              NvoCampo.innerHTML= 
                 ...
                 "     </td>" +
                 "     <td>" +        
                 "          <select  readonly='readonly' id='cmbEstadoRelays_"+intCont+ "'style='width:90px;margin-bottom:5px;'>" +
                 ...
                 "     </td>" +
                 "     <td>" +        
                 "          <select  readonly='readonly' id='cmbTiempoActivacionRelays_"+intCont+ "'style='width:112px;margin-bottom:5px;'>" +
  ...
  function fnAbrirVentanaRelayR(intRenglon,intIndice){
     //Si la tarjeta del DNS es la 2
       if($("#txtTipoTarjetaDNS").val()=='Tarjeta 2')
       {
              ...
              //de la tarjeta 2
              fnAbrirVentanaRelayRTarjeta2(intRenglon,intIndice);
       }
       else //Tarjeta 1
       {
            //Hacer un llamado a la dunción para activar relays
            //de la tarjeta 1
            fnAbrirVentanaRelayRTarjeta1(intRenglon,intIndice);
       }
  }

  //Función que se utiliza para abrir ventanas emergentes por relays del concepto seleccionado
  //cuando el tipo de tarjeta es 2
  function fnAbrirVentanaRelayRTarjeta2(intRenglon,intIndice){

      //Asignar el valor del  tiempo de activación seleccionado
      strValor=$("#txtTiempoActivacionRelays").val();
      strEstadoRelay=$("#txtEstadoRelays").val();
      strRelaysActivar=$("#txtRelaysActivar").val();
      var strUrl='';
      ...
      //Hacer llamada al método para cam
      fnActivarRelaysTarjeta2(intRenglon,1);
      ...
      setTimeout( function() {fnActivarRelaysTarjeta2(intRenglon,2);},  strTiempoCerrar);   
  ...
  //Función que se utiliza para abrir ventanas emergentes por relays del concepto seleccionado
  //cuando el tipo de tarjeta es 2
  function fnActivarRelaysTarjeta2(intRenglon,intNumeroVeces)
  {
      var strRelaysActivar='';
      var intEstadoRelay=$("#txtEstadoRelays").val();
      var intNumeroRelaysT2=$("#txtNumeroRelaysT2").val();
      var strRelaysConceptoT2=$("#txtRelaysConceptoT2").val();
      var strUltimoRelays='';
      var boolActivacionRelays='';

       //Si es la sugunda llamada de la  función
      if(intNumeroVeces==2)
      {
        //Cambiar el estado del relays
        if(intEstadoRelay==0)//Inactivo
        {
          //Activo
          intEstadoRelay=1;
    ...
    var strUrl=$("#txtURLServidor").val()+strRelaysActivar;
     
    //http://videoaccesos.sytes.net:9999/current_state.xml?pw=admin&Relay6=1&Relay7=1&Relay2=1
    eval('ventana'+ intRenglon + "=window.open(strUrl,'ventana'+intRenglon,'width=120px,height=80px,top=1800px,left=1500px,location=no,resizable=no')")
         
   //Cerrar ventana(s) en 2 segundos después de que se cargo la página (para que se realicen los cambios sin inconvenientes) 
      setTimeout( function() {fnCerrarVentanasR(1);
                  $.ajax('catalogos/relaysactivacion/guardar',{
                                            "type" : "post",
                                            "data": {intPrivadaID:$("#txtPrivadaID").val(),
                                                     intDNSID: $("#txtDNSID").val(),
                                                     intRenglonID: intRenglon,
                                                     strRelays:$("#txtRelaysSelT2").val(), 
                                                     strTiempo:$("#txtTiempoActivacionRelays").val(),
                                                     strTipoTarjeta:$("#txtTipoTarjetaDNS").val(),
                                                     strURLTarjeta2:$("#txtURLServidor").val(),
                                            ...
                                            success: function(data){
                                              ...
                                              if(data.resultado==1)
                                              {
                                                 ...
                                                 //Asignar Si para indicar que se guardo la información de la activación de Relays
                                                 boolActivacionRelays='Si';
      ...
      //Si es la segunda llamada de la función y se guardo la información en la BD
      if(intNumeroVeces==2 && boolActivacionRelays=='Si')
      {
  ...
  //Función que se utiliza para abrir ventanas emergentes por relays del concepto seleccionado
  //cuando el tipo de tarjeta es 1
  function fnAbrirVentanaRelayRTarjeta1(intRenglon,intIndice){
