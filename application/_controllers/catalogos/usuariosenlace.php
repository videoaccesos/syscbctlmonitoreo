<?php
	class UsuariosEnlace extends MY_Controller
	{
		var $bolAcceso = FALSE;
		var $data = NULL;
		...
		function __construct () {
	      parent::__construct ();
	      $this->data['strProceso'] = 'usuariosenlace';
	      $this->data['formulario'] = 'USUARIOS DE ENLACE';
	      $this->bolAcceso = $this->Acceso('Usuarios De Enlace');
	      $this->load->model('seguridad/usuarios_model','usuarios');
	    }

		public function index(){
			if($this->bolAcceso)
				$this->data['contenido'] = $this->load->view("catalogos/usuariosenlace",NULL,TRUE);
			else
		...
		public function paginacion(){
			...
			$result = $this->usuarios->filtro(
				                             ...
				                             1, //intPrivadaID, -> control para usuarios del sistema
				                             $config['per_page'],
				                             $config['cur_page']);
		...
		public function guardar(){ 
			...
			$this->form_validation->set_rules('intPrivadaID', 'Privada', 'integer');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if(is_numeric($this->input->post('intUsuarioID'))){
					$res=$this->usuarios->modificar(
						...
						2, //No modificara fechas por default
						0, //intEmpleado -> Se va en '0' cuando son usuarios de enlace
					...
					if(!$res){
						//El nuevo usuario formara parte del Grupo Enlace Sistema Remoto
						$this->load->model('seguridad/gruposusuarios_model','gruposusuarios');
						$this->gruposusuarios->agregar_usuario(0,$this->db->insert_id());
					}
