...
class RecuperacionPatrimonial_Model extends CI_model
{
	...
	public function filtro($strResponsable,$strFechaIni,$strFechaFin,$intEstatusID,$intNumRows,$intPos){
		...
		$this->db->select("R.recuperacion_patrimonial_id, DATE_FORMAT(DATE_ADD(R.fecha, INTERVAL -1 HOUR ),'%d-%m-%Y') AS fecha, R.folio, R.responsable_nombre,
	...
	public function buscar($busqueda = null, $tipo = 1){
		$this->db->select("R.recuperacion_patrimonial_id,R.folio,DATE_FORMAT(DATE_ADD(R.fecha, INTERVAL -1 HOUR ),'%d-%m-%Y %H:%i') AS fecha, R.estatus_id,
			               ...
			               DATE_FORMAT(DATE_ADD(R.aviso_administrador_fecha, INTERVAL -1 HOUR ),'%d-%m-%Y %H:%i') AS fecha_aviso, R.observaciones ", FALSE);
		$this->db->from('recuperacion_patrimonial R');
