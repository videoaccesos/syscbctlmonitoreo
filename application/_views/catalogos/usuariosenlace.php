 ...
 <!-- Modal - FrmUsuarios -->
    <div id="myModal" class="modal hide fade" tabindex="0" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"  >
        ...
        <h3 id="myModalLabel">Usuario de Enlace</h3>
        </div>
            ...
            <form name="frmUsuarios" id="frmUsuarios" action="#" method="post" onsubmit="return(false)" autocomplete="off">
              <div style="display:-webkit-inline-box;">
                  ...
                  <div style="width:240px;">
                    ...
                    <select id="cmbPrivadaID" class="span3"  tabindex="1"></select>
...
//---------- Funciones para la Busqueda
  ...
  function paginacion(){
    ...
    $.post('catalogos/usuariosenlace/paginacion',
...
//--------- Funciones para el Modal
  ...
  function fnBuscar(item){
    if (item.data) {
      $(location).attr('href','catalogos/usuariosenlace/editar/'+item.data[0]);     
    }
    else
      $(location).attr('href','catalogos/usuariosenlace');        
  }
  function fnValidar(){
    return true;
  }
  function fnGuardar(){
    if(fnValidar()) {
      $.post('catalogos/usuariosenlace/guardar',
  ...
  function fnEditar(id){
    $.post('catalogos/usuariosenlace/editar',
                  { intUsuarioID:id
                  },
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
                    if(data.row){
                      ...
                      $('#cmbPrivadaID').val(data.row.privada_id.toString());
                      $('#myModal').modal('show');
                    }
                  }
                 ,
  ...
  function fnEliminar(id){
          $.post('catalogos/usuariosenlace/eliminar',
                  { intUsuarioID:id
                  },
                  function(data) {
                    $('#divMensajes').html(data.mensajes);
