	...
	class Tarjetas extends MY_controller
	{
		...
		public function paginacion(){
			...
			$arr = array('rows' => $result['tarjetas'],
				         ...
				         'total_consignacion' => $result2['total_consignacion'],
				         'total_bajas' => $result2['total_bajas']
				         );
			$this->output->set_content_type('application/json')->set_output(json_encode($arr));
		}
...
public function autocompleteMC(){
		    if(isset($_POST['q']) && isset($_POST['limit'])){
			 	$q = strtolower($_POST['q']);
			 	$limit = $_POST['limit'];
		      	$this->output->set_content_type('application/json')->set_output(json_encode($this->tarjetas->autocompleteMC($q,$limit)));
		    }
		}
	}
 ?>