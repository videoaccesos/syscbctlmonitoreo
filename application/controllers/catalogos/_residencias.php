	...
	class residencias extends MY_controller
	{
		...
		public function autocomplete(){
		    if(isset($_POST['q']) && isset($_POST['limit'])){
			 	...
			 	$limit = $_POST['limit'];
				$rows = array_merge ($this->residencias->autocompleteNroCasa(trim($q),$limit),
				 				     $this->residencias->autocompleteResidente(trim($q),$limit));
		...
		public function autocompleteReportes(){
		    if(isset($_POST['q']) && isset($_POST['limit'])){

			 	$q = strtolower($_POST['q']);
			 	$limit = $_POST['limit'];
				$rows = array_merge ($this->residencias->autocompleteNroCasaReportes(trim($q),$limit),
				 				     $this->residencias->autocompleteResidenteReportes(trim($q),$limit));
		      	$this->output->set_content_type('application/json')->set_output(json_encode($rows));
		    }
		}
