	...
	class Empleados extends MY_controller
	{
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if(is_numeric($this->input->post('intEmpleadoID'))){
					$res=$this->empleados->modificar(
						...
						$this->input->post('strGoogleKey'),
						$this->input->post('intPermisoAdministrador'),
						$this->input->post('intPermisoEncargadoAdministracion'),
						$this->input->post('intSupervisor')
					);	
					$intTipo=1;
				}
				else{
		...
		public function autocomplete(){
		    ...
		    if(isset($_POST['q']) && isset($_POST['limit'])){
			 	$q = strtolower($_POST['q']);
			 	$limit = $_POST['limit'];
		      	$this->output->set_content_type('application/json')->set_output(json_encode($this->empleados->autocomplete_nombre($q,$limit)));
		    }
		}

						public function autocomplete_apellido(){
		    if(isset($_POST['q']) && isset($_POST['limit'])){
			 	$q = strtolower($_POST['q']);
			 	$limit = $_POST['limit'];
		      	$this->output->set_content_type('application/json')->set_output(json_encode($this->empleados->autocomplete_apellido($q,$limit)));
		    }
		}
