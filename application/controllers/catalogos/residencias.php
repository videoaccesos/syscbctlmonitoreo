...
$this->form_validation->set_rules('strTelInterfon', 'Teléfono Interfón', 'max_length[14]');
...
$this->input->post('strTelInterfon'),
...
$this->input->post('strTelInterfon'),
		...
		public function autocomplete($intPrivadaID = 0){
		    if(isset($_POST['q']) && isset($_POST['limit'])){
			 	...
			 	$limit = $_POST['limit'];
				$rows = array_merge ($this->residencias->autocompleteNroCasa(trim($q),$limit,$intPrivadaID),
				 				     $this->residencias->autocompleteResidente(trim($q),$limit,$intPrivadaID));
		...
		public function autocompleteReportes($intPrivadaID = 0){
		    if(isset($_POST['q']) && isset($_POST['limit'])){
			 	$q = strtolower($_POST['q']);
			 	$limit = $_POST['limit'];
				$rows = array_merge ($this->residencias->autocompleteNroCasaReportes(trim($q),$limit,$intPrivadaID),
				 				     $this->residencias->autocompleteResidenteReportes(trim($q),$limit,$intPrivadaID));
		      	$this->output->set_content_type('application/json')->set_output(json_encode($rows));
		    }
		}
		...
		//Método para regresar los datos de una privada que coincide con el id de la residencia 
		//seleccionada (autocomplete).
		public function regresarPrivada(){
			$arrDatos = array('row' => null,
				         'mensajes' => ''
				         );
			//Recuperar el id concatenado de la residencia seleccionada ejemplo:113_N (residenciaId_tipo)
			//En caso de que el tipo sea N-Significa que  la residencia seleccionada correspondiente al
			//autocompleteNroCasa del modelo, de lo contrario pertenece al autocompleteResidente ejemplo:113_Maria Lopez
			$strResidencia = $this->input->post('intResidenciaID');
			//Si existe id, ejecutar el metodo buscarPrivada del modelo 
			//para obtener los datos que corresponden con el id 
			if($strResidencia !=''){
				$arrResultado = $this->residencias->buscarPrivada($strResidencia);
				//Si existen datos, asignar los datos recuperados en el array
				//de lo contrario enviar mensaje para informarle al usuario que no se encontro registro.
				if($arrResultado){
					$arrDatos['row'] = $arrResultado;
				}else {
					$arrDatos['mensajes'] = '<div class="alert alert-danger"><a class="close" data-dismiss="alert">×</a>No se encontro ningún Registro!</div>';	
				}	
			}
			else
				$arrDatos['mensajes'] = '<div class="alert alert-danger"><a class="close" data-dismiss="alert">×</a>Parametro no válido!</div>';
