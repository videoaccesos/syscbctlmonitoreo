<?php 
	class RegistroAccesos extends MY_controller
	{
		...
		function __construct () {
	      ...
	      $this->load->model('catalogos/privadasrelays_model','privadasrelays');
	    }
       ...
       //Función que se utiliza para regresar los relays de la privada seleccionada una vez que se realizo el ping
    	public function regresarRelays(){
        	 ...
        	 //Ejecutar método buscarRelaysDNS para regresar los datos de relays 
	          $row=$this->privadasrelays->buscarRelaysDNS($intPrivadaID,$intDNSID,'');
	          if($row){
		      	   //Regresar los relays del dns URL esta activa
		...
		public function paginacion_consulta(){
			...
			$this->form_validation->set_rules('strSolicitante', 'Solicitante', 'max_length[250]');
			...
			if ($this->form_validation->run()) {
				...
				$res = $this->registroaccesos->filtro_consulta(
												 ...
												 $this->db->escape_str($this->input->post('strSolicitante')),
		...
		public function guardar(){ 
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if(is_numeric($this->input->post('intRegistroAccesoID'))){
					...
					//Llamar el metodo generar imagen 
				}
		...
		public function enviar_notificaciones($intRegistroAccesoID){
									        ...
									        <div class="fluid">
									            <div class="encabezado_back">
									                   ...
									                   <div class="encabezado">
									                   	  <a href="http://www.videoaccesos.net" style="text-decoration:none;">
									                        <img src="http://www.videoaccesos.net/syscbctlmonitoreo/img/logo.png" alt="" style="margin-bottom:10px;">
									                      </a>
									                        <div class="derecha">
			...
			$strMensajePie = '
													<h5>
														...
														En caso de que los datos aquí contenidos no sean reales o usted detecte que la visita notificada no estaba relacionada con su domicilio, por favor notifíquenlo enviando una aclaración a registro@accessbot.info o llamando directamente al teléfono 667 7-59-57-70. Con lo cual nos ayudará a detectar aquellas personas que pretendan falsear su identidad para ingresar al residencial, esto ayudará a controlar más eficientemente el acceso y le proporcionará una mayor tranquilidad.<br><br>
	        ...
	        $strmensaje .= 'Imagen registrada del evento: <br>
							<a href="http://www.videoaccesos.net/detalles_notificacion.php?img='.str_replace(".","%2e",$res['data']['imagen']).'">
							    <img src="http://www.videoaccesos.net/syscbctlmonitoreo/uploads/'.$res['data']['imagen'].'" alt="Imagén del evento!" class="img-polaroid" >
							</a><br>
			...
			$this->email->from('registro@accessbot.info', 'Video Accesos');
...
www.videoaccesos.net
		...
		// Solicitante // filtro entre Residentes, Visitantes y Registro General (para reportes)
		public function autocompleteSolicitantesReportes($intResidenciaID = 0){
		    if(isset($_POST['q']) && isset($_POST['limit'])){
		    	$this->load->model('catalogos/residencias_residentes_model','residentes');
		    	$this->load->model('catalogos/residencias_visitantes_model','visitantes');
		    	$this->load->model('catalogos/registrosgenerales_model','reg_gen');
