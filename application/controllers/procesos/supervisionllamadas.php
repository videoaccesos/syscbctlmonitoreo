<?php 
	class SupervisionLlamadas extends MY_controller
	{
		...
		function __construct () {
	      parent::__construct ();
	      $this->data['strProceso'] = 'supervision_llamadas';
	      $this->data['formulario'] = 'SUPERVISIÓN DE LLAMADAS';
	      $this->bolAcceso = $this->Acceso('SUPERVISION DE LLAMADAS');
	      $this->load->model('procesos/supervicionllamadas_Model','supervisionllamadas');
	    }

		public function index($intRegistroAccesoID){
			...
			$this->data['RegistroAccesoID'] = $intRegistroAccesoID;
			if($this->bolAcceso)
				$this->data['contenido'] = $this->load->view("procesos/supervisionllamadas",$this->data,TRUE);
			else
