<?php 
	class RegistroAccesos extends MY_controller
	{
     ...
     //Función que se utiliza para hacer ping a una url (DNS) para activar o desactivar Relays
	 //de la privada seleccionada
      function ping() { 
	     try
	     {
	     	//Recuperar datos de la vista para hacer ping en el dns seleccionado
	     	$intPrivadaID=$this->input->post('intPrivadaID');
	     	$intDNSID=$this->input->post('intDNSID');
			$strDNS=$this->input->post('strDNS');
	     	//Array con para enviar mensaje y resultado de la conexión
	     	$arr = array('resultado' => null,
	     				 'row' => null,
				         'mensajes' => ''
				         );
	     	//Si dns esta vacio, enviar mensaje
	     	if($strDNS=="")
	     	{
	     		 //URL vacia
				  $arr = array( 'resultado' => '',
				  				'row' => null,
								'mensajes' => '<div class="alert alert-error"><a class="close" data-dismiss="alert">×</a>Favor de seleccionar un DNS, no es posible hacer ping !</div>');

	     	}
	     	else
	     	{
		     	 //Url  a la que se accedera para hacer ping
		     	  $strHost=$strDNS;
		     	  //Asignar puerto
		          $intPuerto= $_SERVER['SERVER_PORT'];
	              //Tiempo de espera
		          $intTiempoEspera=10;
	              //Tiempo final
		     	  $dteFin = microtime(true); 
		     	  //fSockOpen Abre Una Conexión a través de enchufes a Internet oa la ONU Dominio Unix
			      $fpen  = @fSockOpen($strHost, $intPuerto, $errno, $errstr, $intTiempoEspera); 
	              //Tiempo Inicial
			      $dteInicio = microtime(true); 
			      $intCalcularEspera=round((($dteInicio - $dteFin) * 1000), 0)." ms";
	              //Si no hubo conexion 
			      if (!$fpen) { 
			            //URL es desconocida
				      	$arr = array( 'resultado' => 0,
				      				  'row' => null,
								      'mensajes' => '<div class="alert alert-error"><a class="close" data-dismiss="alert">×</a>No es posible hacer conexión, DNS no encontrado  tiempo de espera '.$intCalcularEspera.'!</div>');
			      } 
			      else
			      {
			      	   $row=$this->privadasrelays->buscarRelaysDNS($intPrivadaID,$intDNSID,'');
			      	    //Regresar los relays del dns
			            //URL esta activa
			      	    $arr = array( 
			      	    	          'resultado' => 1,
			      	    	          'row' =>$row,
          ...
          //  echo 'Excepción capturada: ',  $e->getMessage(), "\n";
             $arr = array( 'resultado' => 3,
						          'mensajes' => '<div class="alert alert-error"><a class="close" data-dismiss="alert">×</a>Excepción capturada: '.$e->getMessage().'!</div>');
            $this->output->set_content_type('application/json')->set_output(json_encode($arr));
	      }
