...
export default function PuestosPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editing
        ? `/api/catalogos/puestos/${editing.id}`
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/catalogos/puestos/${deleteTarget.id}`, {
        method: "DELETE",
      });
