...
export default function TurnosPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editing
        ? `/api/catalogos/turnos/${editing.id}`
        : "/api/catalogos/turnos";

      const res = await fetch(url, {
        method: editing ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ descripcion: formDescripcion.trim() }),
      });
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/catalogos/turnos/${deleteTarget.id}`, {
        method: "DELETE",
      });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Administracion del catalogo de turnos</p>
        </div>
        <button
          onClick={openCreate}
