...
// Determina el estado de visualizacion considerando vencimiento
function getEstadoDisplay(item: { estatusId: number; folioTipo: string; fechaVencimiento: string | null }): { label: string; badge: string } {
  // Si esta activa y es folio H con fecha vencimiento pasada -> Vencida
  if (item.estatusId === 1 && item.folioTipo === "H" && item.fechaVencimiento) {
    const hoy = new Date();
    hoy.setHours(0, 0, 0, 0);
    const venc = new Date(item.fechaVencimiento);
    if (venc < hoy) {
      return { label: "Vencida", badge: "bg-orange-100 text-orange-700" };
    }
  }
  return {
    label: ESTATUS_ASIGNACION[item.estatusId] || `Estatus ${item.estatusId}`,
    badge: ESTATUS_BADGE[item.estatusId] || "bg-gray-100 text-gray-700",
  };
}
...
export default function AsignacionTarjetasPage() {
  ...
  /* ---------- tarjetas disponibles (busqueda por lectura) ---------- */
  const fetchTarjetasDisponibles = async (searchQuery?: string) => {
    setLoadingTarjetas(true);
    try {
      const params = new URLSearchParams({ estatusId: "1", limit: "50" });
  ...
  const searchTarjetas = async (query: string) => {
    if (query.length < 2) {
      setTarjetasDisponibles([]);
      return;
    }
    await fetchTarjetasDisponibles(query);
  };
  ...
  const addTarjetaSlot = () => {
    ...
    setTarjetaSearchInputs((prev) => [...prev, ""]);
  };
  ...
  const removeTarjetaSlot = (index: number) => {
    ...
    setTarjetaSearchInputs((prev) => prev.filter((_, i) => i !== index));
                  ...
                  <tr key={`${item.folioTipo}-${item.id}`} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-center">
                      {(() => {
                        ...
                        return (
                          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${estado.badge}`}>
                            {estado.label}
                          </span>
                        );
                      })()}
                      ...
                      <div key={index} className="space-y-1">
                        <div className="flex items-center gap-2">
                          <div className="flex-1 relative">
                            {slot.tarjetaId ? (
                              <div className="flex items-center gap-2 border border-green-300 bg-green-50 rounded-lg px-3 py-2 text-sm">
                                <span className="font-mono font-medium text-green-800">
                                  {selectedTarjeta?.lectura || slot.tarjetaId}
                                </span>
                                <span className="text-xs text-green-600">
                                  ({selectedTarjeta ? (TIPO_TARJETA[selectedTarjeta.tipoId] || "") : ""})
                                </span>
                                <button
                                  type="button"
                                  onClick={() => {
                                    updateTarjetaSlot(index, "tarjetaId", "");
                                    setTarjetaSearchInputs((prev) => {
                                      const n = [...prev];
                                      n[index] = "";
                                      return n;
                                    });
                              ...
                              <>
                                ...
                                <input
                                  type="text"
                                  value={tarjetaSearchInputs[index] || ""}
                                  onChange={(e) => {
                                    const val = e.target.value;
                                    setTarjetaSearchInputs((prev) => {
                                      const n = [...prev];
                                  ...
                                  placeholder="Buscar tarjeta por lectura..."
                            ...
                            {!slot.tarjetaId && (tarjetaSearchInputs[index] || "").length >= 2 && available.length > 0 && (
                              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                                {available.map((t) => (
                                  <button
                                    ...
                                    type="button"
                                    onClick={() => {
                                      updateTarjetaSlot(index, "tarjetaId", String(t.id));
                                      setTarjetaSearchInputs((prev) => {
                                        const n = [...prev];
                                        n[index] = "";
                                        return n;
                                  ...
                                  >
                                    <span className="font-mono font-medium">{t.lectura}</span>
                            ...
                            {!slot.tarjetaId && (tarjetaSearchInputs[index] || "").length >= 2 && !loadingTarjetas && available.length === 0 && (
                              <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-sm text-gray-500">
                                No se encontraron tarjetas disponibles
                              </div>
                            )}
                          </div>
                  ...
                  <div className="mb-4 p-3 rounded-lg bg-gray-50 flex items-center gap-4 text-sm">
                    <div>
                      ...
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        {{ 1: "Disponible", 2: "Asignada", 3: "Danada", 4: "Consignacion", 5: "Baja" }[historialData.tarjeta.estatusId] || "Desconocido"}
                      </span>
                    </div>
                    <div>
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              {(() => {
                                const isVencida = h.estatusId === 1 && h.folioTipo === "H" && h.fechaVencimiento && new Date(h.fechaVencimiento) < new Date(new Date().toDateString());
                                const badge = isVencida ? "bg-orange-100 text-orange-700" :
                                  h.estatusId === 1 ? "bg-green-100 text-green-700" :
                                  h.estatusId === 3 ? "bg-blue-100 text-blue-700" :
                                  "bg-red-100 text-red-700";
                                const label = isVencida ? "Vencida" : h.estatusLabel;
                                return (
                                  <>
                                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${badge}`}>
                                      {label}
                                    </span>
                                    {idx === 0 && h.estatusId === 1 && !isVencida && (
