...
(function($) {
    ...
    $.Autocompleter = function($elem, options) {
        /*
         *   Agregado para busquedas con parametros GET y POST, Autocomplete enlazados
         *   By Alejandro Quiroz
        */
        this.urlQuest = "";
        this.extraPos = 0;
    ...
    $.Autocompleter.prototype.fetchRemoteData = function(filter, callback) {
        ...
        /*    $.ajax({
            ...
            this.urlQuest = this.options.url;
            if(this.options.extraParams.length > 0)
                for(this.extraPos = 0; this.extraPos < this.options.extraParams.length; this.extraPos++){
                    this.urlQuest += '/'+ $(this.options.extraParams[this.extraPos]).val();
                }
            $.post( this.urlQuest,
                 {q:filter,limit:this.options.cacheLength}, 
                  function(data) {
                    if(data)
                        callback(data);
                    else
    ...
    $.Autocompleter.prototype.selectItem = function($li) {
        ...
        if($('#'+this.dom.$elem[0].id+"ID"))
            $('#'+this.dom.$elem[0].id+"ID").val(data);
